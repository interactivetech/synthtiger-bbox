...
class SynthTiger(templates.Template):
    ...
    def generate(self):
        ...
        fg_image, label, bboxes, glyph_fg_image, glyph_bboxes, text_layer = self._generate_text(
            fg_color, fg_style
        )
        print("fg_image: ",fg_image.shape)
        print("fg_image.shape[:2][::-1]: ",fg_image.shape[:2][::-1])
        print("label: ",text_layer.size)
        bg_image = self._generate_background(text_layer.size, bg_color)
        ...
        print(fg_image.shape,bg_image.shape)
    ...
    def _generate_text(self, color, style):
        ...
        # text = "".join(chars)
        # font = self.font.sample({"text": label, "vertical": self.vertical})
        font = self.font.sample()

        char_layers = [layers.TextLayer(char, **font) for char in label] + [layers.TextLayer(char, **font) for char in 'Apple']
        print(len(char_layers))
        # char_layers = layers.Group(
        #     [
        #         [layers.TextLayer(char, **font) for char in chars],
        #         [layers.TextLayer(char, **font) for char in chars]
        #     ]
        # )
        # self.shape.apply(char_layers)
        self.layout.apply(char_layers)
        char_glyph_layers = [char_layer.copy() for char_layer in char_layers]
        print([i for i in char_layers])
        text_layer = layers.Group(char_layers).merge()
        print("text_layer: ",text_layer)
        # text_layer = char_layers
        text_glyph_layer = text_layer.copy()
        # text_layer = layers.Group(char_layers)
        ...
        return out, label, bboxes, glyph_out, glyph_bboxes, text_layer
